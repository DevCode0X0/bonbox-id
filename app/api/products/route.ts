...
async function ensureProducts(db: D1Database) {
  ...
  if (!count?.total) {
    ...
    const inserts = seedProducts.map((product) => db.prepare(`INSERT OR IGNORE INTO products
      (id, name, category, price_label, sales_label, store, commission_rate, commission_label, product_url, affiliate_url, image_url, featured, active, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)`)
...
export async function GET() {
  const { DB } = runtime();
  if (!DB) return Response.json({ products: seedProducts, source: "seed" });
...
export async function PUT(request: Request) {
  ...
  const payload = await request.json() as { id?: string; imageUrl?: string; category?: string };
  ...
  await ensureProducts(DB);
  const result = await DB.prepare("UPDATE products SET image_url = ?, category = COALESCE(NULLIF(?, ''), category), updated_at = ? WHERE id = ?")
    .bind(imageUrl, payload.category?.trim() ?? "", new Date().toISOString(), payload.id).run();
